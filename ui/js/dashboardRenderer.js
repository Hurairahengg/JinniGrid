var DashboardRenderer = (function () {
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            Grid: { color: colors.GridColor, drawBorder: false },
            ticks: {
              color: colors.textColor, font: { family: 'JetBrains Mono', size: 10 },
  ...
  function _fetchDashboardFleet() {
    ...
    ApiClient.getFleetWorkers()
      .then(function (data) {
        ...
        if (workers.length > 0) {
          html += '<div class="compact-fleet-wrapper">';
          html += '<table class="compact-fleet-table">';
          html += '<thead><tr><th>Worker</th><th>State</th><th>Host</th><th>Heartbeat</th><th>Agent</th></tr></thead>';
          html += '<tbody>';
  ...
  function render() {
    ...
    // Portfolio Section
    html += '<section>';
    html += '<div class="section-header"><i class="fa-solid fa-chart-line"></i><h2>Portfolio Overview</h2><span class="section-badge">MOCK</span></div>';
    ...
    html += '<div class="chart-header"><span class="chart-title">Equity Curve</span><span class="chart-period">Last 90 Days</span></div>';
    html += '<div class="chart-wrapper"><canvas id="equityChart"></canvas></div>';
    html += '</div>';

    // Fleet Overview Section
    html += '<section>';
    html += '<div class="section-header"><i class="fa-solid fa-server"></i><h2>Fleet Overview</h2><span class="section-badge">LIVE</span></div>';
  ...
  return { render: render, onThemeChange: onThemeChange, destroy: destroy };
})();
